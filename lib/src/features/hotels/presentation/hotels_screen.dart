...
class _HotelsScreenState extends State<HotelsScreen> {
  ...
  Worker? worker(String id) {
    for (final item in workerRepository.getAll()) {
  ...
  List<HotelAssignment> get assignments {
    ...
    return hotelRepository.getAll().where((a) {
      ...
      final match =
          query.isEmpty ||
          name.contains(query) ||
          a.hotelName.toLowerCase().contains(query) ||
          a.city.toLowerCase().contains(query) ||
          a.room.toLowerCase().contains(query) ||
          a.confirmationCode.toLowerCase().contains(query);
  ...
  Future<void> addAssignment({String? initialWorkerId}) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Alojamiento creado correctamente.')),
    );
  }
  ...
  Future<void> editAssignment(HotelAssignment a) async {
    final value = await Navigator.push<HotelAssignment>(
      context,
      MaterialPageRoute(builder: (_) => HotelFormScreen(assignment: a)),
    );
    if (!mounted || value == null) return;
    hotelRepository.update(value);
    syncWorker(value);
    setState(() {});
  }
  ...
  int count(HotelStatus s) =>
      hotelRepository.getAll().where((a) => a.status == s).length;
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            '${a.city} · Hab. ${a.room} · ${date(a.checkInDate)} → ${date(a.checkOutDate)}',
                                            style: const TextStyle(
                                          ...
                                          if (a.dailyRate > 0)
                                            Text(
                                              '${a.nights} noche(s) · ${money(a.totalCost)}',
                                              style: const TextStyle(
                                                color: Colors.black54,
                                              ),
                                            ),
                                    ...
                                    HotelStatusChip(status: a.status),
          ...
          return Wrap(
            ...
            children: [
              ...
              _card(
                width,
                'Confirmados',
                count(HotelStatus.confirmed).toString(),
                Icons.check_circle,
                AppColors.success,
              ),
              _card(
                width,
                'Check-in',
                count(HotelStatus.checkedIn).toString(),
                Icons.login,
                Colors.blue,
              ),
              _card(
                width,
                'Solicitados',
                count(HotelStatus.requested).toString(),
                Icons.schedule,
                AppColors.warning,
              ),
            ],
    ...
    builder: (context, constraints) {
      ...
      final search = TextField(
        ...
        decoration: const InputDecoration(
          labelText:
              'Buscar trabajador, hotel, ciudad, habitación o confirmación',
          prefixIcon: Icon(Icons.search),
        ),
      );
      final status = DropdownButtonFormField<HotelStatus?>(
        ...
        items: [
          const DropdownMenuItem<HotelStatus?>(
            value: null,
            child: Text('Todos los estados'),
          ),
          ...HotelStatus.values.map(
            (s) =>
                DropdownMenuItem<HotelStatus?>(value: s, child: Text(s.label)),
          ),
        ],
