...
class _HotelFormScreenState extends State<HotelFormScreen> {
  ...
  late final TextEditingController hotelName,
      city,
      address,
      contactName,
      contactPhone,
  ...
  void initState() {
    ...
    checkOutDate =
        a?.checkOutDate ?? DateTime.now().add(const Duration(days: 1));
    ...
    dailyRate = TextEditingController(
      text: a == null ? '' : a.dailyRate.toStringAsFixed(0),
    );
  ...
  void save() {
    ...
    if (!checkOutDate.isAfter(checkInDate)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('El check-out debe ser posterior al check-in.'),
        ),
      );
      return;
    }
    Navigator.pop(
      ...
      HotelAssignment(
        ...
        dailyRate:
            double.tryParse(
              dailyRate.text.replaceAll('.', '').replaceAll(',', '.'),
            ) ??
            0,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(editing ? 'Editar alojamiento' : 'Nuevo alojamiento'),
      ),
      body: Form(
        key: formKey,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (value) {
                          if (value != null) setState(() => workerId = value);
                        },
                      ),
                      ...
                      FilledButton.icon(
                        ...
                        label: Text(
                          editing ? 'GUARDAR CAMBIOS' : 'CREAR ALOJAMIENTO',
                        ),
                      ),
                    ],
                  ),
